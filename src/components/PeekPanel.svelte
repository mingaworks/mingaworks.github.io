<script>
  ...
  // compute whether peek is open from stack
  const isOpen = derived(peekStack, ($peekStack) => $peekStack.length > 0);

  // when stack changes update peekWidth
  // compute a responsive peek width on the client so the peek panel
  // has enough room for the services (.works) content and doesn't
  // force the left menu to become overly narrow.
  ...
  function computeDesiredWidth() {
    try {
      ...
      // On very small screens the peek should effectively be full-height overlay
      if (w < 640) {
        // leave a bit of room for the left menu offset; LayoutSync will use --left-menu-offset
        return Math.max(280, Math.floor(w * 0.85));
      }
      // for larger screens use a fraction of the viewport but clamp to reasonable bounds
  ...
  >
    ...
    <div class="p-6">
      {#if stack.length}
        {#if stack[stack.length -1].type === 'service'}
          <div>
            <h3 class="text-xl font-semibold">{stack[stack.length -1].props.service.title}</h3>
            <p class="text-sm text-gray-600 mt-2">{stack[stack.length -1].props.service.body}</p>
            <div class="mt-6">
              <h4 class="font-semibold">Details</h4>
              <p class="text-sm text-gray-700 mt-2">Individual service detail content goes here. Replace or expand with images, case studies, or additional UI as needed.</p>
            </div>
          </div>
...
<style>
  ...
  height: 100%;
  width: 100%;
    transition: width 300ms ease, transform 220ms ease;
